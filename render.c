...
void do_render(GameState* gameState, SDL_Renderer* renderer, bool* done) {
	...
	if (gameState->statusState == STATUS_STATE_GAMEOVER) {
		Over(gameState, renderer, "Bad End", (SDL_Color) { 210, 8, 45, 255 }, done);
	}
	else if (gameState->statusState == STATUS_STATE_TRUE_END) {
		Over(gameState, renderer, "True End", (SDL_Color) { 200, 112, 126, 255 }, done);
	}
	else if (gameState->statusState == STATUS_STATE_GOOD_END) {
		Over(gameState, renderer, "Good End", (SDL_Color) { 96, 106, 116, 255 }, done);
	}
	else if (gameState->statusState == STATUS_STATE_GAME) {
		// draw background
		drawBackground(gameState, renderer);
		// draw Map
		for (int i = 0; i < gameState->mapCounter; i++) {
			...
			// draw map
			drawMap(gameState, renderer, &gameState->map[i]);
		}
		// draw ledges
		drawLedges(gameState, renderer);
		if (gameState->player.status == 0) { // idle 
			int i = animation_smoothness(12, CHARACTER_FRAMES);
			render_character_animation(gameState, renderer, gameState->idle_anim, WIDTH_PLAYER_IDLE, HEIGHT_PLAYER_IDLE, i);
		}
		else if (gameState->player.status == 1) { // run
			int i = animation_smoothness(8, CHARACTER_FRAMES);
			render_character_animation(gameState, renderer, gameState->run_anim, WIDTH_PLAYER_RUN, HEIGHT_PLAYER_RUN, i);
		}
		else if (gameState->player.status == 2) { // jump
			render_character_animation(gameState, renderer, gameState->jump_anim, WIDTH_PLAYER_JUMP, HEIGHT_PLAYER_JUMP, 0);
		}
		else if (gameState->player.status == 3) { // mid air
			render_character_animation(gameState, renderer, gameState->jump_anim, WIDTH_PLAYER_JUMP, HEIGHT_PLAYER_JUMP, 1);
		}
		else if (gameState->player.status == 4) { // falling
			int i = animation_smoothness(2, CHARACTER_FRAMES) + 2;
			render_character_animation(gameState, renderer, gameState->jump_anim, WIDTH_PLAYER_JUMP, HEIGHT_PLAYER_JUMP, i);
		}
		// draw giga golem
		render_giga_golem(gameState, renderer, gameState->golem);
		// draw status
		render_status(gameState, renderer);
		// Music
		if (!Mix_PlayingMusic()) {
...
void render_character_animation(GameState* gameState, SDL_Renderer* renderer, SDL_Texture* texture[], const int width, const int height, int frame) {
	if (gameState->player.isTakenDamage && gameState->player.isImmortal) {
		...
		SDL_RenderCopyEx(renderer, texture[frame], NULL, &dstRect, 0, NULL, gameState->player.flip);
	}
}
...
void render_giga_golem(GameState* gameState, SDL_Renderer* renderer, SDL_Texture* texture[]) {
	...
	SDL_RenderCopyEx(renderer, texture[0], &srcRect, &dstRect, 0, NULL, 0);
}
...
void render_status(GameState* gameState, SDL_Renderer* renderer) {
	...
	gameState->label_lives = SDL_CreateTextureFromSurface(renderer, textSurface);
	...
	gameState->label_health_potion = SDL_CreateTextureFromSurface(renderer, textSurface);
	SDL_FreeSurface(textSurface);
	...
	SDL_RenderCopy(renderer, gameState->label_lives, NULL, &textRect1);
	SDL_RenderCopy(renderer, gameState->items[0], &srcRect1, &(SDL_Rect) { textRect1.x + textRect1.w, 0, 48, 48 });
	SDL_RenderCopy(renderer, gameState->label_health_potion, NULL, &textRect2);
	SDL_RenderCopy(renderer, gameState->items[0], &srcRect2, &(SDL_Rect) { textRect2.x + textRect2.w, 0, 48, 48 });
}